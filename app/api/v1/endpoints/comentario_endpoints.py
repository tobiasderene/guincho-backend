...
from app.schemas.comentarios import CategoriaVehiculosCreate, CategoriaVehiculosOut

router = APIRouter()

@router.post("/", response_model=CategoriaVehiculosOut, status_code=status.HTTP_201_CREATED)
def crear_comentario(comentario: CategoriaVehiculosCreate, db: Session = Depends(get_db)):
...
@router.get("/publicacion/{id_publicacion}", response_model=List[CategoriaVehiculosOut])
